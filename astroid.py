...
class AsteroidPlanner:
    ...
    def _dp_plan(self, L: int, N: int, P: int, device_ids: List[int]) \
        ...
        # The DP builds partition as start-indices of ALL P stages,
        # e.g. [0, 3, 7, 9] for 4 stages.  HPPPlanConfig expects P-1
        # inter-stage boundaries (stage 0 always starts at 0), so strip
        # the leading 0.
...
def worker(rank: int, cfg: AsteroidConfig,
    ...
    # Derive PP/DP ranks from the plan's device_groups if available,
    # else fall back to the simple formula.
    if plan is not None and plan.device_groups:
        # Build rank → (stage, dp_position) mapping from the planner
        _rank_to_stage: Dict[int, int] = {}
        _rank_to_dp_pos: Dict[int, int] = {}
        _stage_sizes: Dict[int, int] = {}
        ...
        dp_size = _stage_sizes[pp_rank]  # per-stage DP size
        print(f"[RANK {rank}] Using planner device_groups: "
              f"stage_sizes={dict(_stage_sizes)}", flush=True)
    else:
        # Simple uniform mapping: rank = dp_rank * num_stages + pp_rank
    ...
    # ── Create process groups from planner device_groups ─────────────
    ...
    if plan is not None and plan.device_groups:
        # PP groups: devices with the same dp_position across stages
        # form one pipeline (one device per stage).
        ...
        for d in range(max_dp):
            ...
            elif len(pp_ranks) == 1 and rank == pp_ranks[0]:
                # Single-member PP group (this rank has no PP partner)
                grp = torch.distributed.new_group(ranks=pp_ranks)
                pp_process_group = grp
                pp_ranks_in_group = pp_ranks

        # DP groups: all devices assigned to the same stage
        for s in range(pp_size):
        ...
        # Fallback: formula-derived groups (uniform)
    ...
    # ── NCCL Communicators (plan-aware sizing) ────────────────────────
    ...
    # Use planner's partition_points if available, else equal-split fallback
    if plan is not None and plan.partition_points:
        # partition_points[i] = first layer of stage i+1; stage 0 starts at 0
        boundaries = [0] + list(plan.partition_points) + [cfg.num_layers]
        start_layer = boundaries[pp_rank]
        end_layer = boundaries[pp_rank + 1]
        print(f"[RANK {rank}] Using planner partition: boundaries={boundaries}", flush=True)
    else:
        ...
        print(f"[RANK {rank}] Using equal-split fallback", flush=True)
...
if __name__ == "__main__":
    ...
    mp.spawn(worker, args=(cfg, train_data, val_data, plan),
