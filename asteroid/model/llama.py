"""LLaMA transformer blocks — RMSNorm, RotaryEmbedding, SwiGLU, GQA.

Ported from DeviceEmulator/baselines/models/llama.py and adapted for
Asteroid's pipeline-parallel infrastructure.
"""
...
class RotaryEmbedding(nn.Module):
    """Rotary position embedding (RoPE) cache and application."""
...
class LlamaMLP(nn.Module):
    """LLaMA feed-forward with SwiGLU gating."""
...
class LlamaAttention(nn.Module):
    """LLaMA causal attention with optional grouped-query attention (GQA)."""
...
class LlamaBlock(nn.Module):
    """LLaMA pre-norm transformer block using RMSNorm + SwiGLU + RoPE."""
